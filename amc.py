...
class Amc:
    def __init__(self, amcname=None, amccode=None, start_date=None, end_date=None):
        ...
        self.NAV_HISTORY_URL_TEMPLATE = (
            "https://portal.amfiindia.com/DownloadNAVHistoryReport_Po.aspx?tp=1&"
        )
    ...
    def get_start_date(self):
        if os.path.isfile(os.path.join(self.config["cache_path"], "lockdata.json")):
            d = json.load(
                open(os.path.join(self.config["cache_path"], "lockdata.json"))
            )
            try:
                s_date = datetime.datetime.strftime(
                    datetime.datetime.strptime(d[self.name], "%d-%b-%Y")
    ...
    def get_amc_nav_data(self, start_date=None, end_date=None):
        logger.debug(
            f"Before validate call: Start and end date: {start_date} {end_date}"
        )
    ...
    def write_cache_file(self, data=None):
        ...
        today = datetime.datetime.strftime(
            datetime.datetime.strptime(self.end_date, "%d-%b-%Y"), "%s"
        )
