...
class utils:
    ...
    def get_url_data(self, url):
        ...
        try:
            ...
            logger.error(
                'Cannot complete request to {0}. The error was:'.format(url))
            logger.debug(e)
            return None
        return r.text
    ...
    def get_amc_codes(self):
        ...
        if r:
            ...
            amc_codes = {e.string: e.attrs['value']
                         for e in NavDownMFName[0].findAll('option')
                         if e.attrs['value'] != '' and e.string != 'All'}
        else:
            amc_codes = None
        return amc_codes
